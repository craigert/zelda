...
export default function ZeldaGame(){
  ...
  useEffect(()=>{stR.current=init();
    const kd=e=>{kyR.current.add(e.key.toLowerCase());if(["arrowup","arrowdown","arrowleft","arrowright"," "].includes(e.key.toLowerCase()))e.preventDefault();
      ...
      const s=stR.current;if(s&&s.title&&(e.key===" "||e.key==="Enter"||e.key==="z")){
        if(!s.titleReady){s.titleReady=true;Tone.start().then(()=>{initSfx();initAu();});
          if(customMu.title){const a=new Audio(customMu.title);a.loop=true;a.volume=0.5;a.play().then(()=>{ltRef.current="title";customAuRef.current=a;}).catch(()=>{});}
  ...
  return(
    <div ref={wrapRef} style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"linear-gradient(180deg,#060608 0%,#0a0c10 100%)",fontFamily:"monospace",color:"#ccc",padding:16,userSelect:"none",WebkitUserSelect:"none"}}>
      ...
      <canvas ref={cvR} width={W2*4} height={(H2+HH)*4} onClick={()=>{const s=stR.current;if(s&&s.title){if(!s.titleReady){s.titleReady=true;Tone.start().then(()=>{initSfx();initAu();});if(customMu.title){const a=new Audio(customMu.title);a.loop=true;a.volume=0.5;a.play().then(()=>{ltRef.current="title";customAuRef.current=a;}).catch(()=>{});}else{Tone.start().then(()=>{if(!au.i)initAu();playTh("title");ltRef.current="title";}).catch(()=>{});}return;}s.title=false;s.titleReady=false;le(s);ltRef.current=null;return;}if(s&&s.paused)s.paused=false;}} style={{border:"2px solid #222",borderRadius:6,width:W2,height:H2+HH,maxWidth:"100%",boxShadow:"0 0 60px rgba(0,0,0,0.9), 0 0 10px rgba(253,211,51,0.05)"}}/>
