...
function dBo(c,x,y,sz,fl,t,hp,mh,di){c.fillStyle="rgba(0,0,0,0.3)";c.beginPath();c.ellipse(x+sz/2,y+sz-1,sz/2+2,5,0,0,Math.PI*2);c.fill();
  const bc=fl?"#fff":(["#1a7a1a","#bb2211","#2222bb"][di]||"#888");
  const bg=c.createRadialGradient(x+sz*.4,y+sz*.3,2,x+sz/2,y+sz*.5,sz*.4);
  bg.addColorStop(0,fl?"#fff":(["#2aaa2a","#ee4422","#4444dd"][di]||"#aaa"));bg.addColorStop(1,bc);
  c.fillStyle=bg;c.beginPath();c.ellipse(x+sz/2,y+sz*.45,sz*.38,sz*.4,0,0,Math.PI*2);c.fill();
  // Horns
  c.fillStyle=fl?"#eee":"#ff8800";
  c.beginPath();c.moveTo(x+sz*.15,y+sz*.15);c.lineTo(x+sz*.1,y-4);c.lineTo(x+sz*.28,y+sz*.2);c.fill();
  c.beginPath();c.moveTo(x+sz*.85,y+sz*.15);c.lineTo(x+sz*.9,y-4);c.lineTo(x+sz*.72,y+sz*.2);c.fill();
  // Armor lines
  if(!fl){c.strokeStyle="rgba(255,255,255,0.2)";c.lineWidth=1;c.beginPath();c.moveTo(x+sz*.25,y+sz*.4);c.lineTo(x+sz*.75,y+sz*.4);c.stroke();
    c.beginPath();c.moveTo(x+sz*.3,y+sz*.55);c.lineTo(x+sz*.7,y+sz*.55);c.stroke();}
  // Eyes
  const eg2=Math.sin(t/200)*2;
  c.fillStyle=fl?"#333":"#ff0";c.beginPath();c.arc(x+sz*.35,y+sz*.3+eg2,3,0,Math.PI*2);c.fill();c.beginPath();c.arc(x+sz*.65,y+sz*.3+eg2,3,0,Math.PI*2);c.fill();
  if(!fl){c.fillStyle="#f00";c.beginPath();c.arc(x+sz*.35,y+sz*.3+eg2,1.5,0,Math.PI*2);c.fill();c.beginPath();c.arc(x+sz*.65,y+sz*.3+eg2,1.5,0,Math.PI*2);c.fill();}
  // HP bar
  ...
  const hpW=bw*(hp/mh);c.fillStyle="#cc2222";c.fillRect(x-3,y-13,Math.max(0,hpW-2),6);
  c.fillStyle="rgba(255,100,100,0.4)";c.fillRect(x-3,y-13,Math.max(0,hpW-2),3);}
...
export default function ZeldaGame(){
  ...
  useEffect(()=>{if(!muOn){stopMu();ltRef.current=null;return;}
    const ck=()=>{const s=stR.current;if(!s||s.title)return;let th=s.loc.ty==="ow"?"overworld":s.dg[s.loc.di].th;
      if(th!==ltRef.current){ltRef.current=th;
        Tone.start().then(()=>{if(!au.i)initAu();playTh(th);}).catch(()=>{});}};
    ck();const iv=setInterval(ck,500);return()=>{clearInterval(iv);};},[muOn]);
  ...
  function upd(dt){const s=stR.current;if(!s||s.title||s.paused)return;
    ...
    const tm=(px2,py2)=>{const mg=2,l=Math.floor((px2+mg)/TL),r=Math.floor((px2+PS-mg-1)/TL),t2=Math.floor((py2+mg)/TL),b=Math.floor((py2+PS-mg-1)/TL);
      for(let ty=t2;ty<=b;ty++)for(let tx=l;tx<=r;tx++)if(iS(s,tx,ty))return false;return true;};
    const moved=dx!==0||dy!==0;
    if(tm(p.x+dx*sp,p.y))p.x+=dx*sp;if(tm(p.x,p.y+dy*sp))p.y+=dy*sp;
    ...
    for(let i=s.en.length-1;i>=0;i--){const e=s.en[i];e.mt+=dt;if(e.fl>0)e.fl-=dt;
      ...
      if(!eSolid(s,Math.floor((nx+ES/2)/TL),Math.floor((e.y+ES/2)/TL)))e.x=nx;
      if(!eSolid(s,Math.floor((e.x+ES/2)/TL),Math.floor((ny+ES/2)/TL)))e.y=ny;
      e.x=Math.max(TL,Math.min(W2-TL-ES,e.x));e.y=Math.max(TL,Math.min(H2-TL-ES,e.y));
      if(s.sw.a){let sx2=p.x+PS/2,sy2=p.y+PS/2;if(p.dir===0)sy2-=SR;if(p.dir===2)sy2+=SR;if(p.dir===3)sx2-=SR;if(p.dir===1)sx2+=SR;
        if(Math.abs(sx2-ecx)<SR&&Math.abs(sy2-ecy)<SR&&e.fl<=0){e.hp--;e.fl=300;const kb=e.type==="boss"?10:18,kba=Math.atan2(ecy-pcy,ecx-pcx);e.x+=Math.cos(kba)*kb;e.y+=Math.sin(kba)*kb;
      ...
      if(e.hp<=0){s.en.splice(i,1);
        ...
        continue;}
      if(p.ifr<=0&&Math.abs(pcx-ecx)<(PS+ES)/2&&Math.abs(pcy-ecy)<(PS+ES)/2){p.hp--;p.ifr=IFR;sfx("hurt");s.shake.t=300;
  ...
  function drw(t){const cv=cvR.current;if(!cv)return;const c=cv.getContext("2d");const s=stR.current;if(!s)return;
    ...
    // ===== TITLE SCREEN =====
    if(s.title){
      for(let i=0;i<40;i++){const stx=((t/20+i*127)%W2),sty=((i*97+Math.sin(t/1000+i)*20)%FH);
        c.fillStyle=`rgba(255,255,200,${0.3+Math.sin(t/500+i)*0.2})`;c.beginPath();c.arc(stx,sty,0.5+Math.sin(t/300+i*2)*0.3,0,Math.PI*2);c.fill();}
      c.fillStyle="#0a1a08";c.fillRect(0,FH*0.75,W2,FH*0.25);
      c.fillStyle="#0c2010";for(let i=0;i<8;i++){c.beginPath();c.arc(i*68-20,FH*0.75,20+Math.sin(i*3)*8,Math.PI,0);c.fill();}
      const glow=Math.sin(t/400)*0.3+0.7;
      c.fillStyle=`rgba(253,211,51,${glow*0.15})`;c.font="bold 36px monospace";c.textAlign="center";c.fillText("THE LEGEND OF LINK",W2/2,FH*0.28+2);
      c.fillStyle="#fd3";c.fillText("THE LEGEND OF LINK",W2/2,FH*0.28);
      c.fillStyle="#aaa";c.font="13px monospace";c.fillText("Quest for the Triforce",W2/2,FH*0.36);
      c.fillStyle=`rgba(253,211,51,${glow})`;
      c.beginPath();c.moveTo(W2/2,FH*0.43);c.lineTo(W2/2+20,FH*0.53);c.lineTo(W2/2-20,FH*0.53);c.fill();
      c.beginPath();c.moveTo(W2/2-20,FH*0.53);c.lineTo(W2/2,FH*0.63);c.lineTo(W2/2-40,FH*0.63);c.fill();
      c.beginPath();c.moveTo(W2/2+20,FH*0.53);c.lineTo(W2/2+40,FH*0.63);c.lineTo(W2/2,FH*0.63);c.fill();
      if(Math.sin(t/500)>0){c.fillStyle="#fff";c.font="bold 15px monospace";c.fillText("TAP OR PRESS SPACE",W2/2,FH*0.78);}
      c.fillStyle="#555";c.font="10px monospace";c.fillText("WASD move \u00b7 Space attack \u00b7 B bomb \u00b7 M music",W2/2,FH*0.86);
    ...
    // Exit indicators — glowing animated edge portals
    if(m){const[sx2,sy2]=loc.scr.split(",").map(Number);const pulse=Math.sin(t/300)*.5+.5;
      const drawExit=(side)=>{
        let hasExit=false,adjScr;
        if(iD){const dg2=s.dg[loc.di];const[rx,ry]=loc.scr.split(",").map(Number);
          if(side==="N")adjScr=dg2.rooms[`${rx},${ry-1}`];if(side==="S")adjScr=dg2.rooms[`${rx},${ry+1}`];
          if(side==="W")adjScr=dg2.rooms[`${rx-1},${ry}`];if(side==="E")adjScr=dg2.rooms[`${rx+1},${ry}`];
        }else{if(side==="N")adjScr=OW[`${sx2},${sy2-1}`];if(side==="S")adjScr=OW[`${sx2},${sy2+1}`];
          if(side==="W")adjScr=OW[`${sx2-1},${sy2}`];if(side==="E")adjScr=OW[`${sx2+1},${sy2}`];}
        if(!adjScr)return;
        // Check if edge tiles are open
        const openTiles=[];
        if(side==="N")for(let cc=0;cc<CO;cc++){if(!SOLID.has(m[0][cc])&&m[0][cc]!==T.DOOR&&m[0][cc]!==T.BOSS_DOOR)openTiles.push([cc,0]);}
        if(side==="S")for(let cc=0;cc<CO;cc++){if(!SOLID.has(m[RO-1][cc])&&m[RO-1][cc]!==T.DOOR&&m[RO-1][cc]!==T.BOSS_DOOR)openTiles.push([cc,RO-1]);}
        if(side==="W")for(let rr=0;rr<RO;rr++){if(!SOLID.has(m[rr][0])&&m[rr][0]!==T.DOOR&&m[rr][0]!==T.BOSS_DOOR)openTiles.push([0,rr]);}
        if(side==="E")for(let rr=0;rr<RO;rr++){if(!SOLID.has(m[rr][CO-1])&&m[rr][CO-1]!==T.DOOR&&m[rr][CO-1]!==T.BOSS_DOOR)openTiles.push([CO-1,rr]);}
        if(openTiles.length===0)return;
        // Draw glowing strip along open edge tiles
        for(const[ox,oy]of openTiles){
          const px2=ox*TL,py2=oy*TL;
          const glow=c.createRadialGradient(px2+TL/2,py2+TL/2,0,px2+TL/2,py2+TL/2,TL);
          glow.addColorStop(0,`rgba(255,220,80,${pulse*0.25})`);glow.addColorStop(1,"rgba(255,220,80,0)");
          c.fillStyle=glow;c.fillRect(px2-TL/2,py2-TL/2,TL*2,TL*2);
          // Bright edge line
          c.fillStyle=`rgba(255,230,100,${pulse*0.5})`;
          if(side==="N")c.fillRect(px2,py2,TL,3);
          if(side==="S")c.fillRect(px2,py2+TL-3,TL,3);
          if(side==="W")c.fillRect(px2,py2,3,TL);
          if(side==="E")c.fillRect(px2+TL-3,py2,3,TL);
        }
        // Animated chevron arrow
        const mid=openTiles[Math.floor(openTiles.length/2)];const mx=mid[0]*TL+TL/2,my=mid[1]*TL+TL/2;
        const bounce=Math.sin(t/250)*4;
        c.fillStyle=`rgba(255,240,120,${pulse*0.8})`;c.font="bold 20px monospace";c.textAlign="center";
        if(side==="N")c.fillText("\u25b2",mx,my+14+bounce);
        if(side==="S")c.fillText("\u25bc",mx,my-6-bounce);
        if(side==="W")c.fillText("\u25c0",mx+14+bounce,my+7);
        if(side==="E")c.fillText("\u25b6",mx-10-bounce,my+7);
        c.textAlign="left";
      };
      drawExit("N");drawExit("S");drawExit("W");drawExit("E");
    }
    // Enemies
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"linear-gradient(180deg,#060608 0%,#0a0c10 100%)",fontFamily:"monospace",color:"#ccc",padding:16,userSelect:"none",WebkitUserSelect:"none"}}>
