...
export function ae(m,exits){for(const ex of exits){const tp=ex.includes("D")?T.DOOR:ex.includes("B")?T.BOSS_DOOR:T.FLOOR;
  // Single door tile — second tile is FLOOR if open passage, WALL if locked (forces single-door entry)
  const tp2=tp===T.FLOOR?T.FLOOR:T.WALL;
  if(ex[0]==="N"){m[0][7]=tp;m[0][8]=tp2;}if(ex[0]==="S"){m[RO-1][7]=tp;m[RO-1][8]=tp2;}
  if(ex[0]==="E"){m[5][CO-1]=tp;m[6][CO-1]=tp2;}if(ex[0]==="W"){m[5][0]=tp;m[6][0]=tp2;}}}
