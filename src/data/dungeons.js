...
// Scatter grass into some Forest Temple rooms (not all, varying density)
{let seed=77;const rng=()=>{seed=(seed*16807+0)%2147483647;return(seed-1)/2147483646;};
  for(const rk of Object.keys(d1.rooms)){const rm=d1.rooms[rk];if(!rm.tiles)continue;
    // Only ~40% of rooms get vegetation, and density varies per room
    const roomChance=rng();if(roomChance>0.4)continue;
    const density=0.03+rng()*0.15;// 3-18% of floor tiles
    const m=rm.tiles;for(let y=1;y<RO-1;y++)for(let x=1;x<CO-1;x++){
      if(m[y][x]!==T.FLOOR)continue;
      const r=rng();
      if(r<density*0.6)m[y][x]=T.TALLGRASS;
      else if(r<density)m[y][x]=T.GRASS;}}}
...
const d4={name:"Dark Sanctum",color:"#0a0a0a",wc:"#3a1a3a",fc:"#1a0a1a",th:"sanctum",rooms:{
  ...
  "-2,-1":{dark:true,tiles:mr(m=>{
    ...
    // Platforms aligned for cardinal hookshots (right→up→right→up→right)
    // P1: Entry — bottom left (stairs)
    m[8][2]=T.FLOOR;m[8][3]=T.FLOOR;m[9][2]=T.FLOOR;m[9][3]=T.FLOOR;
    m[9][2]=T.STAIRS_DOWN;m[9][3]=T.TORCH;
    // P2: hookshot RIGHT from P1 (same row)
    m[8][6]=T.FLOOR;m[8][7]=T.FLOOR;m[9][6]=T.FLOOR;m[9][7]=T.FLOOR;
    m[8][6]=T.HOOKPOST;m[9][7]=T.HEART;
    // P3: hookshot UP from P2 (same column)
    m[4][6]=T.FLOOR;m[4][7]=T.FLOOR;m[5][6]=T.FLOOR;m[5][7]=T.FLOOR;
    m[5][7]=T.HOOKPOST;m[4][6]=T.TORCH;
    // P4: hookshot RIGHT from P3 (same row)
    m[4][11]=T.FLOOR;m[4][12]=T.FLOOR;m[5][11]=T.FLOOR;m[5][12]=T.FLOOR;
    m[4][11]=T.HOOKPOST;m[5][12]=T.HEART;
    // P5: hookshot UP from P4 — master key island
    m[1][11]=T.FLOOR;m[1][12]=T.FLOOR;m[2][11]=T.FLOOR;m[2][12]=T.FLOOR;
    m[2][12]=T.HOOKPOST;m[1][11]=T.TORCH;
    m[2][11]=T.MASTER_KEY;
