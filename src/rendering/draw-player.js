export function dP(c,x,y,d,t,red){
  const bob=Math.sin(t/150)*0.8;
  const armSwing=Math.sin(t/150)*1.5;
  // Tunic colors — green normally, red with armor upgrade
  const tL=red?"#aa4848":"#48aa48",tD=red?"#7a2a2a":"#2a7a2a";
  const hL=red?"#9a2a2a":"#2a9a2a",hD=red?"#7a1a1a":"#1a7a1a",hB=red?"#882222":"#22882a";
  // Shadow
  ...
  if(d===2){ // FACING DOWN — full front view
    ...
    // Tunic body
    const bg=c.createLinearGradient(x+4,y+8,x+20,y+18);bg.addColorStop(0,tL);bg.addColorStop(1,tD);
    c.fillStyle=bg;
    ...
    // Hat
    const hg=c.createLinearGradient(x+4,y-4,x+20,y+3);hg.addColorStop(0,hL);hg.addColorStop(1,hD);
    c.fillStyle=hg;c.beginPath();c.moveTo(x+5,y+2+bob);c.lineTo(x+12,y-5+bob);c.lineTo(x+19,y+2+bob);c.closePath();c.fill();
    // Hat brim
    c.fillStyle=hB;c.beginPath();c.ellipse(x+12,y+2+bob,8,2,0,0,Math.PI*2);c.fill();
    ...
    c.fillStyle="#8a6a2a";c.fillRect(x+4,y+15+bob,16,2);
    c.fillStyle=tL;c.fillRect(x+1,y+10-armSwing,4,7);c.fillRect(x+19,y+10+armSwing,4,7);
    ...
    // Hat from behind
    const hg2=c.createLinearGradient(x+4,y-4,x+20,y+3);hg2.addColorStop(0,hL);hg2.addColorStop(1,hD);c.fillStyle=hg2;
    ...
    c.fillStyle=hL;c.beginPath();c.moveTo(x+12,y-4+bob);c.quadraticCurveTo(x+20+tailSway,y-2+bob,x+22+tailSway,y+4+bob);c.lineTo(x+18+tailSway,y+3+bob);c.quadraticCurveTo(x+17+tailSway*.5,y-1+bob,x+12,y-3+bob);c.fill();
    ...
    const bg=c.createLinearGradient(x+5,y+8,x+18,y+18);bg.addColorStop(0,tL);bg.addColorStop(1,tD);
    ...
    // Arm forward
    c.fillStyle=tL;c.fillRect(x+2,y+11+bob,4,6);
    ...
    // Hat
    const hg3=c.createLinearGradient(x+2,y-4,x+16,y+3);hg3.addColorStop(0,hL);hg3.addColorStop(1,hD);
    c.fillStyle=hg3;c.beginPath();c.moveTo(x+5,y+2+bob);c.lineTo(x+8,y-5+bob);c.lineTo(x+17,y+2+bob);c.closePath();c.fill();
    c.fillStyle=hB;c.beginPath();c.ellipse(x+11,y+2+bob,7,2,0,0,Math.PI*2);c.fill();
    const ts=Math.sin(t/200)*3;c.fillStyle=hL;c.beginPath();c.moveTo(x+8,y-4+bob);c.quadraticCurveTo(x+16+ts,y-3+bob,x+20+ts,y+2+bob);c.lineTo(x+17+ts,y+1+bob);c.quadraticCurveTo(x+14+ts*.5,y-2+bob,x+9,y-3+bob);c.fill();
    ...
    const bg=c.createLinearGradient(x+6,y+8,x+19,y+18);bg.addColorStop(0,tD);bg.addColorStop(1,tL);
    ...
    // Arm forward
    c.fillStyle=tL;c.fillRect(x+18,y+11+bob,4,6);
    ...
    // Hat
    const hg4=c.createLinearGradient(x+8,y-4,x+22,y+3);hg4.addColorStop(0,hD);hg4.addColorStop(1,hL);
    c.fillStyle=hg4;c.beginPath();c.moveTo(x+7,y+2+bob);c.lineTo(x+16,y-5+bob);c.lineTo(x+19,y+2+bob);c.closePath();c.fill();
    c.fillStyle=hB;c.beginPath();c.ellipse(x+13,y+2+bob,7,2,0,0,Math.PI*2);c.fill();
    const ts2=Math.sin(t/200)*3;c.fillStyle=hL;c.beginPath();c.moveTo(x+16,y-4+bob);c.quadraticCurveTo(x+8-ts2,y-3+bob,x+4-ts2,y+2+bob);c.lineTo(x+7-ts2,y+1+bob);c.quadraticCurveTo(x+10-ts2*.5,y-2+bob,x+15,y-3+bob);c.fill();
  }
}
